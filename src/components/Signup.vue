<template>
    <v-form v-model="valid" ref="form">
        ...
        <v-text-field
            ...
            v-model="confirm"
            min="8"
            :append-icon="conf ? 'visibility' : 'visibility_off'"
            :append-icon-cb="() => (conf = !conf)"
            :type="conf ? 'password' : 'text'"
            :rules="pwdRules"
            counter
            required
            ></v-text-field>
        ...
        methods: {
            submit () {
                if (this.$refs.form.validate()) {
                    ...
                    this.$http.post(config.server.api_url + '/api/v1/auth/signup', params)
                        .then(
                            (resp) => {
